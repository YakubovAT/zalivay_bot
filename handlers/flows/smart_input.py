...
⚠️ МЁРТВЫЙ КОД (временно отключён):
- файл не подключён к активному pipeline хендлеров;
- callbacks из этого файла сейчас не регистрируются в приложении.
Не использовать как источник актуальной логики до отдельной реактивации.
