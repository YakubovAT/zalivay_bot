...
Создание промпта для эталона + классификация категории товара через Text AI.
...
Твои задачи:
1. Определить категорию товара: верх / низ / обувь / головной убор / комбинезон
2. Сформировать промпт для I2I (изоляция товара, удаление фона/модели)
3. Составить краткое описание товара на английском (для создания фото/видео)

Категория «комбинезон» — для цельных изделий: комбинезоны, ромперы, боди с брюками, overalls, jumpsuits.

Отвечай СТРОГО в следующем формате (без markdown):
CATEGORY: [верх|низ|обувь|головной убор|комбинезон]
...
REFERENCE_USER_TEMPLATE = (
    ...
    "- Цвет: {color}\n"
    "- Материал: {material}\n\n"
    "1. Определи категорию: верх / низ / обувь / головной убор / комбинезон\n"
    "   (комбинезон — цельное изделие: комбинезон, ромпер, боди с брюками, jumpsuit, overall)\n\n"
...
VALID_CATEGORIES = {"верх", "низ", "обувь", "головной убор", "комбинезон"}
...
async def generate_reference_prompt(
    ...
    """
    Создает промпт для эталона + категорию + описание товара через T2T AI.
