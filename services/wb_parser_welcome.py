"""
WB парсер для велком флоу.
Парсит товар из Wildberries по артикулу/ссылке.
"""
...
def _img_url(domain: str, basket: int, vol: int, part: int, nmid: int, i: int) -> str:
    return f"https://basket-{basket:02d}.{domain}/vol{vol}/part{part}/{nmid}/images/big/{i}.webp"


def _card_url(domain: str, basket: int, vol: int, part: int, nmid: int) -> str:
    return f"https://basket-{basket:02d}.{domain}/vol{vol}/part{part}/{nmid}/info/ru/card.json"


async def get_product_info(articul: str) -> dict:
    """
    Парсит товар из WB по артикулу.
    ...
    logger.info("WB parser welcome: артикул=%s vol=%s part=%s", articul, vol, part)

    connector = aiohttp.TCPConnector(ssl=False)
    async with aiohttp.ClientSession(connector=connector) as session:
        result = await _find_basket(session, nmid, vol, part)
        if result is None:
            logger.warning("WB parser welcome: корзина не найдена для артикула %s", articul)
            return {}

        basket, domain = result
        logger.info("WB parser welcome: артикул=%s найден в корзине %02d domain=%s", articul, basket, domain)
        ...
        logger.info("WB parser welcome: артикул=%s карточка=%s фото=%d", articul, bool(card), len(images))
